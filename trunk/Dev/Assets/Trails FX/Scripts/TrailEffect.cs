...
namespace TrailsFX
{
    ...
    public partial class TrailEffect : MonoBehaviour
    {
        ...
        public bool active
        {
            ...
            {
                _active = value;
                if (!_active) wasInactive = true;
            }
        }

        [Tooltip(
            "By default, trails are not generated if the renderer is not visibile. This option ignores renderer visibility.")]
        ...
        [Tooltip(
            "Optional mask texture to be applied to the effect. Uses the red channel as an alpha (transparency) multiplier.")]
        ...
        [GradientUsage(hdr: true)] public Gradient colorOverTime;
        ...
        [GradientUsage(hdr: true)] public Gradient colorStartPalette;
        public Camera cam;
        ...
        [Tooltip(
            "Add trails only during these animation states. Optionally include start and end time, example: Attack or Attack(0.1-1.5)")]
        public string animationStates;

        [Tooltip(
            "The animator component. If not specified, first animator component found in children or parent will be used.")]
        public Animator animator;

        public Transform lookTarget;
        public bool lookToCamera = true;
        [Range(0, 1)] public float textureCutOff = 0.25f;
        public OutlineMethod outlineMethod = OutlineMethod.Normals;
        public float rimPower = 2f;
        [Range(0, 1)] public float normalThreshold = 0.3f;
        ...
        const int
            MAX_BATCH_INSTANCES =
                1023; // max number of instances submitted to GPU in a batch. This limit is defined by Unity.
        ...
        [NonSerialized] public bool isSkinned;
        [NonSerialized] public ParticleSystemRenderer particleRenderer;
        [NonSerialized] public bool isParticle;
        int bakeTime;
        int batchNumber;
                ...
                if (isLimitedToAnimationStates)
                {
                    ...
                    for (int k = 0; k < hashCount; k++)
                    {
                        ...
                        if (par1 > par0 && par0 > 0)
                        {
                            ...
                            if (times.Length == 2)
                            {
                                float.TryParse(times[0], System.Globalization.NumberStyles.Any,
                                    System.Globalization.CultureInfo.InvariantCulture, out startTime);
                                float.TryParse(times[1], System.Globalization.NumberStyles.Any,
        ...
        void SetMaterialProperties(Material trailMat)
        {
            ...
            trailMat.SetInt(ShaderParams.ZTest,
                renderOrder == TrailRenderOrder.AlwaysOnTop
        ...
        Vector3 GetSnapshotScale()
        {
            ...
            if (scaleUniform)
            {
                ...
                {
                    return new Vector3(objectScale.x * scale.x * t, objectScale.y * scale.y * t,
                        objectScale.z * scale.z * t);
                }
            }
            else
            {
                ...
                else
                {
                    return new Vector3(
                        ...
                        UnityEngine.Random.Range(scaleStartRandomMin.x, scaleStartRandomMax.x),
                        objectScale.y * scale.y *
                        UnityEngine.Random.Range(scaleStartRandomMin.y, scaleStartRandomMax.y),
                        objectScale.z * scale.z *
                        UnityEngine.Random.Range(scaleStartRandomMin.z, scaleStartRandomMax.z));
                }
            }
        }
        ...
        Vector3 GetRandomizedPosition()
        {
            ...
            if (ground)
            {
                UnityEngine.Ray ray = new UnityEngine.Ray(target.position, Vector3.down);
                RaycastHit hit;
        ...
        void StoreCurrentPositions()
        {
            if (executeInEditMode || Application.isPlaying)
            {
                ...
                if (worldPositionRelativeOption == PositionChangeRelative.OtherGameObject &&
                    worldPositionRelativeTransform != null)
                {
        ...
        void AddSnapshot()
        {
            ...
            if (steps == 0)
            {
                ...
                if (checkScreenPosition)
                {
                    ...
                    int pixelDistance =
                        Mathf.Max(Mathf.CeilToInt(Mathf.Abs(viewportPos1.x - viewportPos0.x) * cam.pixelWidth),
                            Mathf.CeilToInt(Mathf.Abs(viewportPos1.y - viewportPos0.y) * cam.pixelHeight));
                    ...
                    pixelDistance = Mathf.Max((int)(Mathf.Abs(viewportPos1.x - viewportPos0.x) * cam.pixelWidth),
                        (int)(Mathf.Abs(viewportPos1.y - viewportPos0.y) * cam.pixelHeight));
        ...
        void SendToGPU(int meshIndex, int startIndex, int count)
        {
            ...
            if (renderOrder == TrailRenderOrder.DrawBehind && batchNumber == 0 &&
                (theRenderer.isVisible || !ignoreVisibility))
            {
            ...
            if (supportsGPUInstancing)
            {
                for (int s = 0; s < batchMeshSubMeshCount; s++)
                {
                    ...
                    {
                        Graphics.DrawMeshInstanced(batchMesh, s, trailMaterial[batchNumber], matrices, count,
                            properties, UnityEngine.Rendering.ShadowCastingMode.Off, false, layer);
                    }
                }
                ...
                if (clearStencil)
                {
                    ...
                    for (int s = 0; s < batchMeshSubMeshCount; s++)
                    {
                        if (((1 << s) & subMeshMask) != 0)
                        {
                            Graphics.DrawMeshInstanced(batchMesh, s, trailClearMask, matrices, count, null,
            ...
            else
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    for (int s = 0; s < batchMeshSubMeshCount; s++)
                    {
                        if (((1 << s) & subMeshMask) != 0)
                        {
                            Graphics.DrawMesh(batchMesh, matrices[i], trailMaterial[batchNumber], layer, null, s,
                                propertyBlock, false, false);
                        }
                    }
                }
                ...
                if (clearStencil)
                {
                    ...
                    for (int s = 0; s < batchMeshSubMeshCount; s++)
                    {
                        if (((1 << s) & subMeshMask) != 0)
                        {
                            for (int i = 0; i < count; i++)
                            {
                                Graphics.DrawMesh(batchMesh, matrices[i], trailClearMask, layer, null, s, null, false,
                                    false);
                            }
                        }
                    }
                ...
                if (isSkinned || isParticle)
                {
                    ...
                    if (thisFrame != bakeTime)
                    {
                        ...
                        else
                        {
                            ...
                            particleRenderer.BakeMesh(meshPool[meshPoolIndex],
