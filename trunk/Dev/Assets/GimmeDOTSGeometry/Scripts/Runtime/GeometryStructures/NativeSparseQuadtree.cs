...
namespace GimmeDOTSGeometry
{
    public unsafe struct NativeSparseQuadtree<T> : IQuadtree<T>, IDisposable where T : unmanaged, IEquatable<T>
    {
        ...
        [NoAlias] private NativeList<int> free;
        ...
        public NativeSparseQuadtree(Rect bounds, int maxDepth, int capacity, Allocator allocator)
            : this(new float3(bounds.center.x, 0.0f, bounds.center.y), new float2(bounds.width, bounds.height),
                maxDepth, capacity, allocator)
        {
        }
        ...
        public NativeSparseQuadtree(float3 position, float2 scale, int maxDepth, int capacity, Allocator allocator)
        {
            ...
            {
                Debug.LogError(
                    "Max depth can't be more than 16, as it would not be possible to lay a z-order curve with 32-bits precision");
            }
        ...
        public struct GetCellsInRadiusJob : IJob
        {
            ...
            [NoAlias, ReadOnly] public NativeList<QuadtreeNode> quadtreeNodes;

            [NoAlias, WriteOnly] public NativeList<uint> result;
                    ...
                    for (int i = 0; i < rectangleBuffer.Length; i++)
                    {
                        ...
                        if (subRect.Overlaps(circleRect))
                        {
                            ...
                            if (nextNode != 0)
                            {
                                ...
                                this.GetCellsInRadiusRecursion(this.quadtreeNodes[nextNode], subRect, depth + 1,
                                    nextCode);
                            }
                        }
                    }
        ...
        /// <returns></returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void GetCellsInRadius(float2 center, float radius, ref NativeList<uint> result)
        {
            // var bottomLeft = this.bottomLeft;
            // //var  center = center;
            // var maxDepth = this.maxDepth;
            // var quadtreeNodes = this.quadtreeNodes;
            // //var  radius = radius;
            // //var  result = result;
            ...
            var rect = Rect.MinMaxRect(min.x, min.y, max.x, max.y);
            GetCellsInRadiusRecursion(this.quadtreeNodes[0], rect, 0, 0, center, radius, radiusSq);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        void GetCellsInRadiusRecursion(QuadtreeNode node, Rect parentRect, int depth, uint currentCode, float2 center,
            float radius, float radiusSq)
        {
            if (depth == maxDepth)
            {
                if (ShapeOverlap.RectangleCircleOverlap(parentRect, center, radiusSq))
                {
                    //this.result.Add(currentCode);
                }
            }
            else
            {
                ...
                var circleMin = center - radius;
                var circleMax = center + radius;
                ...
                for (int i = 0; i < rectangleBuffer.Length; i++)
                {
                    ...
                    if (subRect.Overlaps(circleRect))
                    {
                        ...
                        {
                            uint nextCode = (currentCode | (uint)(i << (inverseDepth * D)));
                            this.GetCellsInRadiusRecursion(quadtreeNodes[nextNode], subRect, depth + 1,
                                nextCode, center, radius, radiusSq);
                        }
                    }
                }
            }
        ...
        /// <param name="dependsOn"></param>
        /// <returns></returns>
        public JobHandle GetCellsInRadius(float2 center, float radius, ref NativeList<uint> result,
        ...
        public struct GetCellsInRadiiJob : IJobParallelFor
        {
            ...
            [NoAlias, ReadOnly] public NativeArray<float2> centers;
            [NoAlias, ReadOnly] public NativeArray<float> radii;

            [NoAlias, ReadOnly] public NativeList<QuadtreeNode> quadtreeNodes;

            [NoAlias, WriteOnly] public NativeParallelHashSet<uint>.ParallelWriter result;

            private float radiusSq;

            private void GetCellsInRadiusRecursion(int index, QuadtreeNode node, Rect parentRect, int depth,
                uint currentCode)
            {
                if (depth == this.maxDepth)
                {
                ...
                else
                {
                    ...
                    for (int i = 0; i < rectangleBuffer.Length; i++)
                    {
                        ...
                        if (subRect.Overlaps(circleRect))
                        {
                            ...
                            {
                                uint nextCode = (currentCode | (uint)(i << (inverseDepth * D)));
                                this.GetCellsInRadiusRecursion(index, this.quadtreeNodes[nextNode], subRect, depth + 1,
                                    nextCode);
                            }
                        }
                    }
        ...
        public struct GetCellsInRectangleJob : IJob
        {
            ...
            private void GetCellsInRectangleRecursion(QuadtreeNode node, float2 min, float2 max, int depth,
                uint currentCode)
            {
                if (depth == this.maxDepth)
                {
                ...
                else
                {
                    ...
                    for (int i = 0; i < rectangleBuffer.Length; i++)
                    {
                        ...
                        if (subRect.Overlaps(this.rect))
                        {
                            ...
                            {
                                uint nextCode = (currentCode | (uint)(i << (inverseDepth * D)));
                                this.GetCellsInRectangleRecursion(this.quadtreeNodes[nextNode], subRect.min,
                                    subRect.max, depth + 1, nextCode);
                            }
                        }
                    }
                }
        ...
        public struct GetCellsInRectanglesJob : IJobParallelFor
        {
            ...
            [NoAlias, ReadOnly] public NativeArray<Rect> rectangles;

            [NoAlias, ReadOnly] public NativeList<QuadtreeNode> quadtreeNodes;

            [NoAlias, WriteOnly] public NativeParallelHashSet<uint>.ParallelWriter result;

            private void GetCellsInRectangleRecursion(int index, QuadtreeNode node, float2 min, float2 max, int depth,
                uint currentCode)
            {
                if (depth == this.maxDepth)
                {
                ...
                else
                {
                    ...
                    for (int i = 0; i < rectangleBuffer.Length; i++)
                    {
                        ...
                        if (subRect.Overlaps(this.rectangles[index]))
                        {
                            ...
                            {
                                uint nextCode = (currentCode | (uint)(i << (inverseDepth * D)));
                                this.GetCellsInRectangleRecursion(index, this.quadtreeNodes[nextNode], subRect.min,
                                    subRect.max, depth + 1, nextCode);
                            }
                        }
                    }
        ...
        public bool IsCreated => this.dataBuckets.IsCreated || this.quadtreeNodes.IsCreated || this.free.IsCreated ||
                                 this.count.IsCreated;
