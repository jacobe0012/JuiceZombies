...
namespace Rewired.Utils
{
    ...
    public class ExternalTools : IExternalTools
    {
        ...
        {
            get { return _getPlatformInitializerDelegate; }
            set { _getPlatformInitializerDelegate = value; }
        }
...
#if !REWIRED_XBOXONE_DISABLE_VIBRATION
            if(_xboxOne_gamepadDLLException) return;
            Rewired.Platforms.XboxOne.XboxOneGamepadMotorType motor =
 (Rewired.Platforms.XboxOne.XboxOneGamepadMotorType)motorInt;
            try {
                switch(motor) {
        ...
        public bool XboxOne_Gamepad_SetGamepadVibration(ulong xboxOneJoystickId, float leftMotor, float rightMotor,
            float leftTriggerLevel, float rightTriggerLevel)
        {
            return false;
        }

        public void XboxOne_Gamepad_PulseVibrateMotor(ulong xboxOneJoystickId, int motorInt, float startLevel,
            float endLevel, ulong durationMS)
        {
        }
#endif
        ...
        public void GetDeviceVIDPIDs(out List<int> vids, out List<int> pids) {
            ...
            try {
                ...
                int[] ids = null;
                using(AndroidJavaObject jniArray =
 android_view_InputDevice.CallStatic<AndroidJavaObject>("getDeviceIds")) {
                    if(jniArray != null) {
 ...
 android_view_InputDevice.CallStatic<AndroidJavaObject>("getDevice", ids[i])) {
                            if(jo == null) continue;
        ...
        public void WindowsStandalone_ForwardRawInput(System.IntPtr rawInputHeaderIndices,
            System.IntPtr rawInputDataIndices, uint indicesCount, System.IntPtr rawInputData, uint rawInputDataSize)
        {
#if UNITY_2022_PLUS
            ...
            Rewired.Internal.Windows.Functions.ForwardRawInput(rawInputHeaderIndices, rawInputDataIndices, indicesCount,
                rawInputData, rawInputDataSize);
#else
