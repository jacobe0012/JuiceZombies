...
public abstract class MB3_MeshBakerRoot : MonoBehaviour
{
        ...
        public void SortByDistanceAlongAxis(List<GameObject> gos)
        {
            ...
            for (int i = 0; i < gos.Count; i++)
            {
                if (gos[i] != null)
                {
                    ...
                    item.point.z = gos[i].GetComponent<Renderer>().sortingOrder;
            ...
            items.Sort((a, b) => { return a.point.z.CompareTo(b.point.z); });
    ...
    [HideInInspector] public abstract MB2_TextureBakeResults textureBakeResults { get; set; }
    ...
    public static bool DoCombinedValidate(MB3_MeshBakerRoot mom, MB_ObjsToCombineTypes objToCombineType,
        MB2_EditorMethodsInterface editorMethods, MB2_ValidationLevel validationLevel)
    {
        ...
        if (mom is MB3_MeshBakerCommon)
        {
            ...
            {
                Debug.LogWarning(
                    "Texture Bake Result on this component is not the same as the Texture Bake Result on the MB3_TextureBaker.");
            }
        }
        ...
        for (int i = 0; i < objsToMesh.Count; i++)
        {
            ...
            if (go == null)
            {
                Debug.LogError(string.Format(
                    "The list of objects to combine contains a null at position {0}. Select and use [shift + delete] to remove the object, or purge all null objects from the context menu.",
                    i));
                return false;
            }
            ...
            if (m != null)
            {
                    ...
                    validationLevel >= MB2_ValidationLevel.robust)
                {
                    ...
                    {
                        Debug.LogWarning("Object " + objsToMesh[i] +
                                         " in the list of objects to combine has overlapping submeshes (submeshes share vertices). If the UVs associated with the shared vertices are important then this bake may not work. If you are using multiple materials then this object can only be combined with objects that use the exact same set of textures (each atlas contains one texture). There may be other undesirable side affects as well. Mesh Master, available in the asset store can fix overlapping submeshes.");
                    }
                }
            }
        ...
        if (mom is MB3_MeshBaker)
        {
            ...
            if (mom is MB3_MeshBaker && ((MB3_MeshBaker)mom).meshCombiner.settings.renderType ==
