    ...
    class PhysicsShapeAuthoringEditor : BaseEditor
    {
        static class Styles
        {
            ...
            static readonly GUIContent k_FitToRenderMeshesWarningLabelPl = new GUIContent(
                ...
                EditorGUIUtility.Load("console.warnicon") as Texture,
                L10n.Tr(
                    $"{k_Plural} has non-uniform scale. Trying to fit the shape to render meshes might produce unexpected results.")
            );
            ...
            static readonly string[] k_StaticColliderStatusMessage =
            {
                ...
                L10n.Tr(
                    $"{k_Plural} will be considered static. Add a {ObjectNames.NicifyVariableName(typeof(PhysicsBodyAuthoring).Name)} component if you will move them at run-time.")
            };
            ...
            public static readonly string BoxCapsuleSuggestion =
                L10n.Tr(
                    $"Target {ShapeType.Box} has uniform size on its two short axes and a large convex radius. Consider using a {ShapeType.Capsule} instead.");
            ...
            public static readonly string BoxSphereSuggestion =
                L10n.Tr(
                    $"Target {ShapeType.Box} has uniform size and large convex radius. Consider using a {ShapeType.Sphere} instead.");

            public static readonly string CapsuleSphereSuggestion =
                L10n.Tr(
                    $"Target {ShapeType.Capsule}'s diameter is equal to its height. Consider using a {ShapeType.Sphere} instead.");

            public static readonly string CylinderCapsuleSuggestion =
                L10n.Tr(
                    $"Target {ShapeType.Cylinder} has a large convex radius. Consider using a {ShapeType.Capsule} instead.");
...
#pragma warning restore 649
        ...
        void UpdateStatusMessages()
        {
                ...
                switch (shape.ShapeType)
                {
                    case ShapeType.Box:
                        ...
                        else if (math.abs(box.BevelRadius - min * 0.5f) < k_Epsilon)
                        {
                            ...
                            else if (math.abs(math.lengthsq(box.Size - new float3(min)) - math.pow(max - min, 2f)) <
                                     k_Epsilon)
            ...
            var hierarchyStatus =
                StatusMessageUtility.GetHierarchyStatusMessage(targets, out var hierarchyStatusMessage);
                ...
                switch (shape.ShapeType)
                {
                        ...
                        EditorGUI.BeginChangeCheck();
                    {
                        ...
                        var normal = math.cross(math.normalizesafe(forward), math.normalizesafe(right))
                                     * 0.5f * math.lerp(math.length(right) * size2.x, math.length(forward) * size2.y,
                                         0.5f);
