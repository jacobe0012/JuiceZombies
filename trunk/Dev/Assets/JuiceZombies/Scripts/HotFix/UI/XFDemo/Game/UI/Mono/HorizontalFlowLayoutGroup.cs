...
public class HorizontalFlowLayoutGroup : LayoutGroup
{
        ...
        for (var i = 0; i < rectChildren.Count; i++)
        {
            ...
            if (currentColumnHeight + childHeight > workingHeight)
            {
                ...
                if (!layoutInput)
                {
                    ...
                    LayoutColumn(_columnList, currentColumnHeight, currentColumnWidth, workingHeight, padding.top, w,
                        axis);
                }
    ...
    protected void LayoutColumn(IList<RectTransform> contents, float columnHeight, float columnWidth, float maxHeight,
        float yOffset, float xOffset, int axis)
    {
        var yPos = yOffset;
