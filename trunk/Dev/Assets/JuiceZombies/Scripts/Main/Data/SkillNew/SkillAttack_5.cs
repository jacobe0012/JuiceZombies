...
{
    //回旋镖-追踪
    public partial struct SkillAttack_5 : ISkillAttack
    {
        /// <summary>
        /// 0
        /// </summary>
        ...
        public LocalTransform DoSkillMove(ref SkillAttackData_ReadWrite refData, in SkillAttackData_ReadOnly inData)
        {
            ...
            var moveTime = totalDuration - stayDuration;
            var firstTime = totalDuration - stayDuration - backDuration;
            float3 tweenPos = temp.Position;

            //Debug.Log($"attack5 tttttttt  {t}");
            //Debug.Log($"tweenPos  {tweenPos}");
            if (isHit)
            {
                if (!isBack)
                {
                    isBack = true;
                    hp = (int)trackPar.y == 0 ? MathHelper.MaxNum : (int)trackPar.y;
                }

                float3 towards = math.normalizesafe(new float3(MathHelper.RotateVector(dir.xy, 180), 0));
                // if (refData.storageInfoFromEntity.Exists(caster))
                // {
                //     var tempTowards = math.normalizesafe(refData.cdfeLocalTransform[caster].Position - temp.Position);
                //     if (math.length(tempTowards) > math.EPSILON)
                //     {
                //         towards = tempTowards;
                //     }
                // }
                ...
                t = math.remap(totalDuration - stayDuration - backDuration, totalDuration, 0, d, t);
                temp.Rotation = MathHelper.LookRotation2D(towards);
                tweenPos = MathHelper.EaseInQuad(t, b, c, d);
            }
            else
            {
                if (t < firstTime)
                {
                    ...
                    var c = dir * speed *
                            (firstTime);
                    var d = firstTime;
                    t = math.remap(0, firstTime, 0, d, t);
                    tweenPos = MathHelper.EaseOutQuad(t, b, c, d);
                    //Debug.Log($"attack5 tweenPos  {tweenPos}");
                    //b
                    //Debug.LogError($"t < moveTime / 2f  {t} c{math.length(c)}");
                }
                else if (t >= firstTime && t <= firstTime + stayDuration)
                {
                    stayPos = temp.Position;
                }
                else if (t > firstTime + stayDuration)
                {
                    isHit = true;
                    //t = t - (moveTime / 2f + stayDuration);

                    //float3 towards = math.normalizesafe(math.mul(temp.Rotation, MathHelper.picForward));

                    //float3 towards = math.normalizesafe(math.mul(temp.RotateZ(180).Rotation, math.up()));
                    float3 towards = math.normalizesafe(new float3(MathHelper.RotateVector(dir.xy, 180), 0));
                    // if (refData.storageInfoFromEntity.Exists(caster))
                    // {
                    //     var tempTowards = math.normalizesafe(refData.cdfeLocalTransform[caster].Position - temp.Position);
                    ...
                    tweenPos = MathHelper.EaseInQuad(t, b, c, d);
                    temp.Rotation = MathHelper.LookRotation2D(towards);
                    //Debug.LogError($"t > moveTime / 2f + stayDuration  {t} towards {towards} c{math.length(c)}");
                }
            }
        ...
        public void OnStart(ref SkillAttackData_ReadWrite refData, in SkillAttackData_ReadOnly inData)
        {
            ...
            backDuration = math.max(trackPar.z / 1000f, inData.fDT);


            // Debug.Log(
            //     $"attack5 stayDuration{stayDuration} backDuration{backDuration} backhp{trackPar.y} duration{duration} hp{hp}");
            duration += backDuration + stayDuration;
        ...
        public void OnHit(ref SkillAttackData_ReadWrite refData, in SkillAttackData_ReadOnly inData)
        {
            ...
            if (hp == 1 && !isHit)
            {
                //Debug.Log($"attack5 {hp}");
                isHit = true;
                stayPos = refData.cdfeLocalTransform[inData.entity].Position;
                hp = (int)trackPar.y + 1;
            }
