...
{
    //折射的弹幕
    public partial struct SkillAttack_3 : ISkillAttack
    {
        /// <summary>
        /// 0
        /// </summary>
        ...
        public LocalTransform DoSkillMove(ref SkillAttackData_ReadWrite refData, in SkillAttackData_ReadOnly inData)
        {
            ...
            //UnityEngine.Debug.LogError($"pos:{temp.Position},scale:{temp.Scale}");
            return temp;
        }
        ...
        {
            Debug.LogError($"attack3 on Hit {target}");
            ...
            if (inData.cdfeBoardData.HasComponent(target))
            {
                var boardData = inData.cdfeBoardData[target];
                float3 originPos = refData.cdfeLocalTransform[inData.entity].Position;
                float3 normal = default;
                switch (boardData.type)
                {
                    case BoardTypeEnum.Up:
                        originPos.y += 1;
                        break;
                    case BoardTypeEnum.Down:
                        originPos.y += -1;
                        break;
                    case BoardTypeEnum.Left:
                        originPos.x += 1;
                        break;
                    case BoardTypeEnum.Right:
                        originPos.x += -1;
                        break;
                }

                normal = math.normalizesafe(originPos - refData.cdfeLocalTransform[inData.entity].Position);

                dir = MathHelper.Reflect(dir, normal);
                Debug.LogError($"dir board  {dir}");
            }
            else if (inData.cdfeObstacleTag.HasComponent(target))
            {
                var normal = math.normalizesafe(refData.cdfeLocalTransform[inData.entity].Position
                                                - refData.cdfeLocalTransform[target].Position);
                // 计算反射方向
                dir = MathHelper.Reflect(dir, normal);
                Debug.LogError($"dir obs  {dir}");
            }
            else
            {
                hp--;
