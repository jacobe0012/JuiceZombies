...
{
    //定时掉血 300012
    public partial struct SkillAttack_10 : ISkillAttack
    {
        /// <summary>
        /// 0
        /// </summary>
        ...
        {
            UnityEngine.Debug.LogError($"OnHit1");
        ...
        {
            int currentTick = (int)((trackPar.x / 1000f) / inData.fDT);

            if (tick % currentTick == 0)
            {
                //Debug.LogError($"定时掉血{tick}");
                hp--;
                BuffHelper.AddTriggerForBullet(ref refData, in inData, inData.sortkey, triggerID, caster, target,
                    MathHelper.Forward(refData.cdfeLocalTransform[inData.entity].Rotation));
            }
        }
