...
{
    //围绕旋转 索敌后发射
    public partial struct SkillAttack_11 : ISkillAttack
    {
        /// <summary>
        /// 0
        /// </summary>
        ...
        public LocalTransform DoSkillMove(ref SkillAttackData_ReadWrite refData, in SkillAttackData_ReadOnly inData)
        {
            ...
            if (!isSeeked)
            {
                ...
                currentAngle += trackPar[1] * inData.fDT;
                float radian = math.radians(currentAngle);
                quaternion rotation = quaternion.RotateZ(radian);
                float3 offset = new float3(trackPar[0] / 1000f, 0, 0); // 初始偏移（沿 X 轴）
                float3 rotatedOffset = math.mul(rotation, offset); // 旋转后的偏移

                // 计算新位置，保持 Y 坐标不变

                float3 newPosition = refData.cdfeLocalTransform[caster].Position + rotatedOffset;

                temp.Position = newPosition;
            }
            else if(!isEmit&&isSeeked)
            {
                isEmit = true;
                if(!refData.storageInfoFromEntity.Exists(inData.entity)|| !refData.storageInfoFromEntity.Exists(followedEntity))
                {
                    currentAngle += trackPar[1] * inData.fDT;
                    float radian = math.radians(currentAngle);
                    quaternion rotation = quaternion.RotateZ(radian);
                ...
                defaultDir =math.normalize(  refData.cdfeLocalTransform[followedEntity].Position- refData.cdfeLocalTransform[inData.entity].Position);
                var acc = acceleration / 1000f;
                speed += acc * inData.fDT;
                temp.Position += defaultDir * speed * inData.fDT * actionSpeed;
            ...
            //var oldDir= refData.cdfeLocalTransform[inData.entity].Position - refData.cdfeLocalTransform[caster].Position;
            //var pos = refData.cdfeLocalTransform[caster].Position+ math.normalizesafe(oldDir) * trackPar[0] / 1000f;
            //var temp = refData.cdfeLocalTransform[inData.entity];
            //temp.Position = pos;
            ////temp= temp.RotateZ(math.radians(trackPar[1] * inData.fDT));
            //temp= MathHelper.RotateAround(temp, refData.cdfeLocalTransform[caster].Position, math.radians(trackPar[1] * inData.fDT));
        ...
        public void OnStart(ref SkillAttackData_ReadWrite refData, in SkillAttackData_ReadOnly inData)
        {
            ...
            dir= refData.cdfeLocalTransform[inData.entity].Position - refData.cdfeLocalTransform[caster].Position;
            
            currentAngle = math.degrees(math.atan2(dir.y, dir.x));
        ...
        public void OnUpdate(ref SkillAttackData_ReadWrite refData, in SkillAttackData_ReadOnly inData)
        {
            if (!isSeeked&&tick%10==0)
            {
              ...
              ref inData.config.value.Value.configTbbullets.configTbbullets;
                ...
                var targetLoc = BuffHelper.SeekTargets(ref bullets.searchPara, ref bullets.searchPara, bullets.searchType, 11, -1,
                     ref bullets.searchPara,
                     inData.allEntities, caster, inData.entity,
                     refData.cdfeLocalTransform, refData.cdfeTargetData,inData.cdfeChaStats, (uint)lockInt, out targetPos,
                     out float3 targetDir, out followedEntity, refData.bfeBuff,inData.cdfeBulletTempTag, (uint)(inData.timeTick), 1);
