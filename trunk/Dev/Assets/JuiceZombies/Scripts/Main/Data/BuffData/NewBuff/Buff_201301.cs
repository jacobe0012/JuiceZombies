...
namespace Main
{
    //元素专用 地形火 障碍物油
    //new 
    public partial struct Buff_201302 : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        public void OnTick(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            for (int i = 0; i < entities.Length; i++)
            {
                ...
                if (refData.cdfeChaStats.HasComponent(entity) && !inData.cdfeMapElementData.HasComponent(entity) && refData.storageInfoFromEntity.Exists(entity))
                {
                    ...
                    if (dis <= limitDis)
                    {
                        refData.ecb.AppendToBuffer<Buff>(inData.sortkey, entity, new Buff_127 { id = 127, permanent = false, carrier = entity, caster = default, duration = argsNew.args1.x / 1000f }.ToBuff());
                    }
                }
            }
