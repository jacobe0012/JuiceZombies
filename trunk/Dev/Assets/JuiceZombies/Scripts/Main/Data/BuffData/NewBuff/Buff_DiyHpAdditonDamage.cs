...
namespace Main
{
    //diy buff 对满血敌人额外造成50%伤害
    //new 
    public partial struct Buff_DiyHpAdditonDamage : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        {
            //argsNew.args1.x hp下限 argsNew.args1.y hp上限 argsNew.args1.z 伤害加成  都是万分比

            var defenderHpRatios = refData.cdfeChaStats[refData.damageInfo.defender].chaResource.hp /
                                   (float)refData.cdfeChaStats[refData.damageInfo.defender].chaProperty.maxHp;
            if (defenderHpRatios <= argsNew.args1.y / 10000f && defenderHpRatios >= argsNew.args1.z / 10000f)
            {
                //Debug.LogError($"defenderHpRatios {defenderHpRatios}");
                refData.damageInfo.damage.RatiosDamage(argsNew.args1.x / 10000f);
            }
        }
