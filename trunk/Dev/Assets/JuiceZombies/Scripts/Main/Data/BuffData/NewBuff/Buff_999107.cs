...
namespace Main
{
    //恢复之风
    //new 
    public partial struct Buff_999107 : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        public void OnOccur(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            tickTime = (int)(monsterfeature.para[1] / 1000f / inData.fdT);
            hpLimit = monsterfeature.para[0] / 10000f * refData.cdfeChaStats[carrier].chaProperty.maxHp;
        }
        ...
        {
            if (refData.cdfeChaStats[carrier].chaResource.hp <= hpLimit && !isActive)
            {
                isActive = true;
                activeTick = timeElapsed;
            }

            if (isActive && (timeElapsed - activeTick) % tickTime == 0)
            {
                int index = 0;
                ...
                int maxLevel = monsterfeature.para[3];
                if (level < maxLevel)
                {
                    CureCarrier(ref refData, inData, hpRations);
                    level++;
                }
            }
        }
        ...
        private void CureCarrier(ref BuffData_ReadWrite refData, BuffData_ReadOnly inData, int hpRations)
        {
            var damage = new Damage
            {
                normal = -(int)(refData.cdfeChaStats[carrier].chaProperty.maxHp * hpRations / 10000f),
            };
