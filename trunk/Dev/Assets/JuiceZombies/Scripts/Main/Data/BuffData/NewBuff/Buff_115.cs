...
namespace Main
{
    //恐惧    状态
    //new 
    public partial struct Buff_115 : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        public void OnOccur(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            {
                //SetImmuneControl(ref refData, inData);
                var temp = refData.cdfeChaStats[carrier];
                temp.chaControlState.forcedmove.x = true;
                temp.chaControlState.attacker = caster;
            ...
            {
                if (battleConstConfig[i].constantName == (FixedString128Bytes)"status_lock_speed")
                {
                    //移动伤害的移动值
                    fixedSpeed = battleConstConfig[i].constantValue / 1000f;
                    break;
                }
            }
            refData.ecb.AppendToBuffer<Buff>(inData.sortkey, carrier, new Buff { CurrentTypeId = Buff.TypeId.Buff_109, Single_3 = duration });
        ...
        public void OnRemoved(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            if (refData.cdfeChaStats.HasComponent(carrier))
            {
                //SetImmuneControl(ref refData, inData);
                var temp = refData.cdfeChaStats[carrier];
                temp.chaControlState.forcedmove.x = false;
                temp.chaControlState.attacker = Entity.Null;
                refData.cdfeChaStats[carrier] = temp;
            }
        }
        ...
        {
            var temp = refData.cdfeChaStats[carrier];
            temp.chaResource.curMoveSpeed = fixedSpeed * 1000f;
            refData.cdfeChaStats[carrier] = temp;
        }
