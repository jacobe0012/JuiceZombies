...
namespace Main
{
    //回复生命 30000020  参数：固定数值，万分比
    public partial struct Buff_30000020 : IBuffOld
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        {
            Debug.Log(
                $"{-(int)math.floor(refData.cdfeChaStats[carrier].chaProperty.maxHp * (buffArgs.args1 / 10000f) + buffArgs.args0)}");

            var healNum = (int)math.floor(refData.cdfeChaStats[carrier].chaProperty.maxHp * (buffArgs.args1 / 10000f) +
                                          buffArgs.args0);
            var lostHp = refData.cdfeChaStats[carrier].chaProperty.maxHp - refData.cdfeChaStats[carrier].chaResource.hp;
            ...
            refData.ecb.AppendToBuffer(inData.sortkey, inData.wbe, new DamageInfo
            {
                ...
                {
                    normal =
                        -math.clamp(healNum, 0,
                            refData.cdfeChaStats[carrier].chaProperty.maxHp -
