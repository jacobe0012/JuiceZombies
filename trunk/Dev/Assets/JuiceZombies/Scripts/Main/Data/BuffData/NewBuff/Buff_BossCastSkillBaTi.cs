...
{
    //霸体buff
    public partial struct Buff_BossCastSkillBaTi : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        public void OnOccur(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            {
                var mass = refData.cdfePhysicsMass[carrier];
                var velocity = refData.cdfePhysicsVolocity[carrier];
                var agent = refData.cdfeAgentBody[carrier];
                mass.InverseMass = 1 / 9999f;
                velocity.Angular = 0;
                velocity.Linear = 0;
                agent.Stop();
                var state = refData.cdfeChaStats[carrier];
                if (!state.chaImmuneState.immunePush)
                {
                    state.chaImmuneState.immunePush = true;
                }

                //UnityHelper.ChangeProperty(refData.ecb, inData.sortkey, ref refData.cdfeChaStats, ref refData.cdfePlayerData, ref refData.cdfePhysicsMass, inData.cdfeLocalTransform, 99999);
                refData.cdfePhysicsMass[carrier] = mass;
                refData.cdfePhysicsVolocity[carrier] = velocity;
                refData.cdfeChaStats[carrier] = state;
        ...
        public void OnRemoved(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            if (refData.cdfeChaStats.HasComponent(carrier))
            {
                ...
                if (state.chaImmuneState.immunePush)
                {
                    state.chaImmuneState.immunePush = false;
                }

                mass.InverseMass = 1f / state.chaProperty.mass;
                refData.cdfeChaStats[carrier] = state;
                refData.cdfePhysicsMass[carrier] = mass;
            }
        }
