...
{
    //时间回溯
    public partial struct Buff_BackTrackTime : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        {
            var pos = inData.cdfeLocalTransform[inData.entity].Position;
            var bfeBackTrackTimes = refData.bfeBackTrackTimeBuffer[inData.entity];
            int count = (int)(backTimeDuration / inData.fdT);
            if (bfeBackTrackTimes.Length < count)
            {
                bfeBackTrackTimes.Add(new BackTrackTimeBuffer
                {
                    position = pos
                });
            }
