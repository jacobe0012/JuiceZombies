...
namespace Main
{
    //血之契约
    //new 
    public partial struct Buff_999111 : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        public void OnBeHurt(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            var monsterID = monsterfeature.para[1];
            var hpRations = monsterfeature.para[0];

            Debug.Log($"111refData.subDamage:{refData.damageInfo.damage.normal}");    
            if (refData.damageInfo.damage.normal > refData.cdfeChaStats[carrier].chaResource.hp ||
                refData.cdfeChaStats[carrier].chaResource.hp <=
                refData.cdfeChaStats[carrier].chaProperty.maxHp * (hpRations / 10000f))
            {
                refData.damageInfo.damage.normal = MathHelper.MaxNum;
                //refData.damageInfo.damage.normal = 0;
                //var cha = refData.cdfeChaStats[carrier];
                //cha.chaResource.hp = 0;
                ...
                BuffHelper.GenerateItemAndAddData(inData.cdfePostTransformMatrix, inData.player, ref refData.ecb, inData.cdfeLocalTransform, inData.cdfeWeaponData,
                    refData.cdfeChaStats, refData.cdfePhysicsMass, refData.cdfeAgentLocomotion,
                    refData.cdfeEnemyData, inData.globalConfigData, inData.prefabMapData, inData.gameTimeData,
                    inData.gameOthersData, inData.sortkey, 2, monsterID, 0,
                    inData.cdfeLocalTransform[carrier].Position,0, isinBoss, bossScenSize);
            }
        }
