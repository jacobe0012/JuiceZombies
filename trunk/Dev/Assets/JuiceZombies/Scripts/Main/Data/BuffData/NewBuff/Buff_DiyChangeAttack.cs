...
namespace Main
{
    //改变boss攻击方式    状态
    //new 
    public partial struct Buff_DiyChangeAttack : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        /// <summary>
        /// 用来处理ontick的逻辑, tickTime=0时不执行ontick 15
        /// </summary>
        public int tickTime;
        ...
        {
            if (refData.cdfeEnemyData.HasComponent(carrier))
            {
                var temp = refData.cdfeEnemyData[carrier];
                attackOldValue.x = temp.isHitBackAttack == true ? 1 : 0;
                attackOldValue.y = (int)temp.attackType;
                temp.isHitBackAttack = true;
                if (argsNew.args1.x == 2)
                {
                    temp.attackType = EnemyAttackType.CollideAttack;
                }
        ...
        public void OnRemoved(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            {
                var temp = refData.cdfeEnemyData[carrier];
                temp.isHitBackAttack = attackOldValue.x == 0 ? false : true;
                if (argsNew.args1.x == 2)
                {
                    temp.attackType = (EnemyAttackType)attackOldValue.y;
                }
