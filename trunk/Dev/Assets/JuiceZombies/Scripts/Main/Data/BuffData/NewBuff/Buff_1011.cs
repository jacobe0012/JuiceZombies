...
namespace Main
{
    //二次攻击冷却标记
    //new 
    public partial struct Buff_1011 : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        {
            if (inData.bfeSkill.HasBuffer(carrier))
            {

                var weaponSkillID= refData.cdfePlayerData[carrier].playerOtherData.weaponSkillId;

                float time = 0f;
                ref var skillconfig=ref  inData.globalConfigData.value.Value.configTbskills.configTbskills;
                for(int i = 0; i < skillconfig.Length; i++)
                {
                    if (skillconfig[i].id == weaponSkillID)
                    {
                        time = skillconfig[i].cd/1000f * (1 + skillconfig[i].reductionCd * refData.cdfeChaStats[carrier].chaProperty.defaultcoolDown);
                    }
                }
                var skills = inData.bfeSkill[carrier];
                for (int i = 0; i < skills.Length; i++)
                {
                    if (skills[i].Int32_0 == weaponSkillID)
                    {
                        duration = skills[i].Single_3+time;
                        break;
                    }
                }
            }
