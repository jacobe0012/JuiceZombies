...
namespace Main
{
    //受损    状态
    //new 
    public partial struct Buff_122 : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        public void OnOccur(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            {
                if (battleConstConfig[i].constantName == (FixedString128Bytes)"status_damaged_rate")
                {
            ...
            if (temp.chaProperty.maxHp > temp.chaProperty.defaultMaxHp)
            {
                hpReduction = (int)(temp.chaProperty.defaultMaxHp * (1 - judgeRate));
            }
            else
            {
                hpReduction= (int)(temp.chaProperty.maxHp * (1 - judgeRate));
            }
            temp.chaResource.hp = temp.chaProperty.maxHp - hpReduction;
        ...
        public void OnRemoved(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            var temp = refData.cdfeChaStats[carrier];
            temp.chaResource.hp +=hpReduction;
            refData.cdfeChaStats[carrier] = temp;
        ...
        public void SetImmuneControl(ref BuffData_ReadWrite refData, BuffData_ReadOnly inData)
        {
            var temp = refData.cdfeChaStats[carrier];
            temp.chaControlState.counterMove = false;
            temp.chaControlState.cantMove = false;
            temp.chaControlState.cantWeaponAttack = false;
            temp.chaControlState.forcedmove = false;
            refData.cdfeChaStats[carrier] = temp;
        }
