...
namespace Main
{
    //盗贼
    //new 
    public partial struct Buff_999105 : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        public void OnHit(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            int index = 0;
            ref var monsterFeatures = ref inData.globalConfigData.value.Value.configTbmonster_features.configTbmonster_features;
            ...
            var money = monsterfeature.para[0];
            var currentMoney = refData.cdfePlayerData[refData.damageInfo.defender].playerData.exp;

            if (refData.cdfePlayerData.HasComponent(refData.damageInfo.defender) )
            {
                if(currentMoney >= money)
                {
                    UnityHelper.ChangeProperty(ref refData.ecb, inData.sortkey, ref refData.cdfeChaStats,
                    ref refData.cdfePlayerData, ref refData.cdfePhysicsMass, refData.cdfeLocalTransform, 201200, -money, refData.damageInfo.defender);
                }
                else
                {
                    UnityHelper.ChangeProperty(ref refData.ecb, inData.sortkey, ref refData.cdfeChaStats,
                    ref refData.cdfePlayerData, ref refData.cdfePhysicsMass, refData.cdfeLocalTransform, 201200, -currentMoney, refData.damageInfo.defender);
                }
