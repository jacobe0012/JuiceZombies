...
{
    /// <summary>
    /// 增加id 1014 的状态，攻击力万分比伤害，不可叠加，有特效
    /// </summary>
    public partial struct Buff_1015 : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        public void OnOccur(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            {
                Debug.Log($"101 texiao id {id}");
                var ins = UnityHelper.SpawnBuffSpecialEffect(ref refData.ecb, inData.sortkey, inData.eT,
                    inData.cdfeLocalTransform,
                    inData.gameTimeData, inData.globalConfigData, specialEffectId, inData.prefabMapData, carrier);
                if (ins != Entity.Null)
                {
            ...
            // }
            var entityGroup = inData.cdfeEntityGroupData[carrier];

            var sort = refData.cdfeJiYuSort[entityGroup.renderingEntity];
            sort.value.y += 1;
            refData.cdfeJiYuSort[entityGroup.renderingEntity] = sort;
            Debug.Log($"1015duration{duration}");
        }
        ...
        public void OnRemoved(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            sort.value.y -= 1;
            refData.cdfeJiYuSort[entityGroup.renderingEntity] = sort;
            // var trancarrier = refData.cdfeLocalTransform[carrier];
            // var trancaster = refData.cdfeLocalTransform[caster];
            // trancarrier.Position.y -= trancaster.Scale / 2f + trancaster.Scale / 2f;
            // refData.cdfeLocalTransform[carrier] = trancarrier;
        ...
        public void OnTick(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            if (inData.cdfeMapElementData.HasComponent(caster))
            {
                ...
                damage.normal = (int)math.floor(refData.cdfeChaStats[caster]
                                                    .chaProperty.atk *
                                                argsNew.args1.x /
                                                10000f);
            }
            ...
            UnityEngine.Debug.Log($"101OnTick{timeElapsed}  {tickTime}");
        ...
        {
            //Debug.Log($"101OnPerUnitMove");
            //AddMoveDamage(ref refData, inData);
        }

        private void AddMoveDamage(ref BuffData_ReadWrite refData, BuffData_ReadOnly inData)
        {
            if (!refData.cdfeChaStats.HasComponent(carrier))
            {
                return;
            ...
            {
                if (battleConstConfig[i].constantName == (FixedString128Bytes)"status_move_damage_atk")
                {
                    //移动伤害的移动值
                    damageRate = battleConstConfig[i].constantValue / 10000f;
                    break;
                }
            }

            var damageValue = damageRate * refData.cdfeChaStats[carrier].chaProperty.maxHp;
            ...
            if (!refData.storageInfoFromEntity.Exists(carrier)) return;
            if (inData.cdfeMapElementData.HasComponent(caster))
            {
                tags.environmentDamage = true;
                damage.normal = (int)math.floor(damageValue);
            }
            else
            {
                tags.directDamage = true;
            ...
            Debug.Log($"101OnPerUnitMove dam");
            refData.ecb.AppendToBuffer(inData.sortkey, inData.wbe, new DamageInfo
            {
                attacker = caster,
                defender = carrier,
