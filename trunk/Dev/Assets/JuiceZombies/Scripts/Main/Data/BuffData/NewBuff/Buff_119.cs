...
namespace Main
{
    //狂战    状态
    //new 
    public partial struct Buff_119 : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        public void OnOccur(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            {
                if (battleConstConfig[i].constantName == (FixedString128Bytes)"status_vampirism_rate")
                {
                    //状态-吸血固定比例
                    vampirismRate = battleConstConfig[i].constantValue / 10000f;
                    break;
                }
            }
        ...
        public void OnTick(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            //{
            //    damage.environment = (int)math.floor(refData.cdfeChaStats[carrier]
            //                                             .chaProperty.maxHp *
            //                                         argsNew.args1.x /
            //                                         10000f);
            //}
        ...
        public void OnHit(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            if (refData.damageInfo.tags.directHeal || refData.damageInfo.tags.propsHeal) return;
            ...
            var damageValue = (int)math.floor((refData.damageInfo.damage.normal /
                                              refData.cdfeChaStats[carrier].chaProperty.atk) *
                                              vampirismRate * refData.cdfeChaStats[carrier].chaProperty.maxHp);
            damage.normal = -math.abs(damageValue);
            UnityEngine.Debug.LogError($"伤害{refData.damageInfo.damage.normal}，回血：{damageValue},vampirismRate:{vampirismRate}");
            refData.ecb.AppendToBuffer(inData.sortkey, inData.wbe, new DamageInfo
            {
                attacker = carrier,
                defender = carrier,
