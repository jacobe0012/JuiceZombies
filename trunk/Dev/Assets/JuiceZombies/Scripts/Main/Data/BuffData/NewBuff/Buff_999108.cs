...
namespace Main
{
    //小巨人
    //new 
    public partial struct Buff_999108 : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        {
            if (timeElapsed == (int)(3f / inData.fdT))
            {
                int index = 0;
                ...
                var scaleRations = monsterfeature.para[0];
                ...
                UnityHelper.ChangeProperty(ref refData.ecb, inData.sortkey, ref refData.cdfeChaStats,
                    ref refData.cdfePlayerData, ref refData.cdfePhysicsMass, refData.cdfeLocalTransform, 222100,
                    scaleRations, carrier);
                UnityHelper.ChangeProperty(ref refData.ecb, inData.sortkey, ref refData.cdfeChaStats,
                    ref refData.cdfePlayerData, ref refData.cdfePhysicsMass, refData.cdfeLocalTransform, 208020,
                    massRations, carrier);
                UnityHelper.ChangeProperty(ref refData.ecb, inData.sortkey, ref refData.cdfeChaStats,
                    ref refData.cdfePlayerData, ref refData.cdfePhysicsMass, refData.cdfeLocalTransform, 202020,
                    hpRations, carrier);
                if (refData.cdfeChaStats[carrier].chaProperty.scaleRatios !=
                    refData.cdfeChaStats[carrier].chaProperty.scaleRatios)
                {
                    refData.ecb.AddComponent(inData.sortkey, carrier, new PushColliderData
                    {
                        toBeSmall = refData.cdfeChaStats[carrier].chaProperty.scaleRatios <
                                    refData.cdfeChaStats[carrier].chaProperty.scaleRatios,
                        initScale = refData.cdfeLocalTransform[carrier].Scale,
                        targetScale = refData.cdfeLocalTransform[carrier].Scale *
                                      (refData.cdfeChaStats[carrier].chaProperty.scaleRatios / 10000f)
                    });
                }
            }
        }
