...
namespace Main
{
    //急速    状态
    //new 
    public partial struct Buff_117 : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        public void OnOccur(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            {
                if (battleConstConfig[i].constantName == (FixedString128Bytes)"status_beyond_speed")
                {
                    //状态-急速-移动速度加成
            ...
            for (int i = 0; i < effectConfig.Length; i++)
            {
                ...
                {
                    oldScale = effectConfig[i].sizeX> effectConfig[i].sizeY ? (int)(effectConfig[i].sizeX/1000f) : (int)(effectConfig[i].sizeY/1000f);
                    break;
                }
            }
        ...
        public void OnTick(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            if (effectEntity != default)
            {
                if (temp.chaResource.curMoveSpeed > temp.chaProperty.defaultMaxMoveSpeed * judgeRate && effectEntity != default)
                {
