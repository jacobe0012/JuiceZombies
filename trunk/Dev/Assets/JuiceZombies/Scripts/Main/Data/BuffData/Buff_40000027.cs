...
namespace Main
{
    //生成荆棘 用于减速玩家 间隔五秒  args0为道具id args1为道具持续时间
    public partial struct Buff_40000027 : IBuffOld
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        {
            //Debug.Log("OnPerUnitMove--------------------");
            var prefab = inData.prefabMapData.prefabHashMap["skill_thron"];
            var entity = refData.ecb.Instantiate(inData.sortkey, prefab);
            var loc = inData.cdfeLocalTransform[carrier];
            var newLoc = new LocalTransform
            {
                Position = loc.Position,
                Scale = inData.cdfeLocalTransform[prefab].Scale * buffArgs.args0,
                Rotation = loc.Rotation,
            };
            refData.ecb.SetComponent(inData.sortkey, entity, newLoc);
            //refData.ecb.AddComponent(inData.sortkey, entity,
            //    new SkillAttackData
            //    {
            //        data = new SkillAttack_Thistles
            //        {
            //            id = id,
            //            duration = 5f,
            //            tick = 0,
            ...
            {
                buffID = 30000003,
                buffArgs = new float3x4 { c0 = new float3(buffArgs.args2, 0, 0) }
            });
        }
