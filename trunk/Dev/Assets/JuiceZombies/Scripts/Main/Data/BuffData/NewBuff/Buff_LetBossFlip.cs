...
namespace Main
{
    ...
    /// 用于boss放技能时能够移动
    /// </summary>
    public partial struct Buff_LetBossFlip : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        public void OnOccur(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            if (refData.cdfeChaStats.HasComponent(carrier))
            {
                ...
                state.chaControlState.bossCanFlip = true;
            ...
            // {
            //     var buff = buffs[i];
            //     if (buff.CurrentTypeId == Buff.TypeId.Buff_BaTi)
            //     {
        ...
        public void OnRemoved(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            if (refData.cdfeChaStats.HasComponent(carrier))
            {
                ...
                state.chaControlState.bossCanFlip = false;
                refData.cdfeChaStats[carrier] = state;
            }

            // if (refData.cdfeAgentBody.HasComponent(carrier))
            // {
        ...
        public void OnUpdate(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            // {
            //     var agent = refData.cdfeAgentBody[carrier];
            //     agent.SetDestination(refData.cdfeLocalTransform[inData.player].Position);
            //     refData.cdfeAgentBody[carrier] = agent;
            // }
        }
