...
namespace Main
{
    //生成道具(仅适用于battle_item表)  args0为道具id args1为道具持续时间
    public partial struct Buff_30000015 : IBuffOld
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        {
            FixedString128Bytes propName = (FixedString128Bytes)"";
            ref var propArray = ref inData.globalConfigData.value.Value.configTbbattle_items.configTbbattle_items;
            for (int k = 0; k < propArray.Length; k++)
            {
                if (propArray[k].id == buffArgs.args0)
                {
            ...
            if (buffArgs.args1 != 0)
            {
                refData.ecb.AddComponent(inData.sortkey, propEntity,
                    new SkillAttackData
                    {
                        data = new SkillAttack_0
                        {
                            id = buffArgs.args0 + inData.sortkey,
