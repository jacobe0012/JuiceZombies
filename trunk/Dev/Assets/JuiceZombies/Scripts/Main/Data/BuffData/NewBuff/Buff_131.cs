...
namespace Main
{
    //混乱 改变索敌   状态
    //new 
    public partial struct Buff_131 : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        public void OnOccur(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            oldTarget = refData.cdfeEnemyData[carrier].targetGroup;
            var enemydata = refData.cdfeEnemyData[carrier];
            enemydata.targetGroup = (uint)math.pow(2, argsNew.args1.x);
            // if (argsNew.args1.x == 1||argsNew.args1.x==3)
            // {
            //     enemydata.target = inData.player;
            // }
            // else if(argsNew.args1.x == 5)
            // {
            //     float distance = 0f,mindistance=9999f;
            //     ///找距离最近的boss
            //     for (int i = 0; i < inData.entities.Length; i++)
            //     {
            //         var entity = inData.entities[i];
            //         if (inData.cdfeTargetData[entity].BelongsTo == math.pow(2, 5))
            //         {
            //            
            //             distance = math.distance(refData.cdfeLocalTransform[carrier].Position, refData.cdfeLocalTransform[entity].Position);
            //             if (mindistance > distance)
            //             {
            //                 mindistance = distance;
            //                 enemydata.target = entity;
            //             }
            //         }
            //     }
            // }
            refData.cdfeEnemyData[carrier] = enemydata;
            ...
            ref var effectConfig =
                ref inData.globalConfigData.value.Value.configTbspecial_effects.configTbspecial_effects;
            ...
            for (int i = 0; i < effectConfig.Length; i++)
            {
                ...
                {
                    oldScale = effectConfig[i].sizeX > effectConfig[i].sizeY
                        ? effectConfig[i].sizeX
                        : effectConfig[i].sizeY;
                    break;
                }
            }
        }
        ...
        {
            var temp = refData.cdfeEnemyData[carrier];
            temp.targetGroup = oldTarget;
            refData.cdfeEnemyData[carrier] = temp;
        }
