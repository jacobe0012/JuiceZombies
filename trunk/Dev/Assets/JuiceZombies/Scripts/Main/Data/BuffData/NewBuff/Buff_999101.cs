...
namespace Main
{
    //怪物特性 嗜血
    //new 
    public partial struct Buff_999101 : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        public void OnUpdate(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            ref var monsterFeatures =
                ref inData.globalConfigData.value.Value.configTbmonster_features.configTbmonster_features;
            ...
            var hpLimit = monsterfeature.para[0];
            var attRations = monsterfeature.para[1];
            var speedRations = monsterfeature.para[2];
            if (refData.cdfeChaStats[inData.player].chaResource.hp <=
                refData.cdfeChaStats[inData.player].chaProperty.maxHp * hpLimit / 10000f)
            {
                if (!isActive)
                {
                    Debug.Log($"101 isActive, attRations:{attRations},speedRations:{speedRations}");
                    isActive = true;
                    UnityHelper.ChangeProperty(ref refData.ecb, inData.sortkey, ref refData.cdfeChaStats,
                        ref refData.cdfePlayerData, ref refData.cdfePhysicsMass, refData.cdfeLocalTransform, 203020,
                        attRations, carrier);
                    UnityHelper.ChangeProperty(ref refData.ecb, inData.sortkey, ref refData.cdfeChaStats,
                        ref refData.cdfePlayerData, ref refData.cdfePhysicsMass, refData.cdfeLocalTransform, 207020,
                        speedRations, carrier);
                }
            }
            ...
            else
            {
                if (isActive)
                {
                    Debug.Log($"101 is  not Active, attRations:{attRations},speedRations:{speedRations}");
                    UnityHelper.RemoveChangeProperty(ref refData.ecb, inData.sortkey, ref refData.cdfeChaStats,
                        ref refData.cdfePlayerData, ref refData.cdfePhysicsMass, inData.cdfeLocalTransform, 203020,
                        attRations, carrier);
                    UnityHelper.RemoveChangeProperty(ref refData.ecb, inData.sortkey, ref refData.cdfeChaStats,
                        ref refData.cdfePlayerData, ref refData.cdfePhysicsMass, inData.cdfeLocalTransform, 207020,
                        speedRations, carrier);
