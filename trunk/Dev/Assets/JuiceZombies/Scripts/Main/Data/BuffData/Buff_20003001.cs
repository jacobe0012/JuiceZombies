...
namespace Main
{
    ...
    /// 减速
    /// </summary>
    public partial struct Buff_20003001 : IBuffOld
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        {
            //Debug.Log($"OnOccur 减速 20003001 ,speed00000:{refData.cdfeAgentLocomotion[carrier].Speed},enemy{carrier}");
            var temp = refData.cdfeChaStats[carrier];

            temp.chaProperty.maxMoveSpeed = (int)(temp.chaProperty.maxMoveSpeed * (1 - buffArgs.args0 / 10000f));
            //temp.chaProperty.speedRecoveryTime = (int)(temp.chaProperty.speedRecoveryTime * (1 + buffArgs.args0 / 10000f));
            ...
            if (refData.cdfeAgentLocomotion.HasComponent(carrier))
            {
                ...
                agent.Speed *= (1 - buffArgs.args0 / 10000f);

                refData.cdfeAgentLocomotion[carrier] = agent;
            }

            //Debug.Log($"OnOccur 减速 20003001 ,speed:{refData.cdfeAgentLocomotion[carrier].Speed},enemy:{carrier}");
        }

        public void OnRemoved(ref BuffOldData_ReadWrite refData, in BuffOldData_ReadOnly inData)
        {
            //Debug.Log($"OnRemoved 减速 20003001");
            var temp = refData.cdfeChaStats[carrier];
