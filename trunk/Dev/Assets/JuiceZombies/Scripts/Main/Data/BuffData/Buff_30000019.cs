...
namespace Main
{
    //拾取道具 30000019   参数：拾取范围，拾取道具1，拾取道具2...
    public partial struct Buff_30000019 : IBuffOld
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        {
            //Debug.LogError($"Buff_30000019");
            //Debug.LogError($"Buff_30000019 {buffArgs.args0}");
            for (int i = 0; i < inData.dropsEntities.Length; i++)
            {
                if (math.length(inData.cdfeLocalTransform[inData.dropsEntities[i]].Position -
                                inData.cdfeLocalTransform[inData.player].Position) > buffArgs.args0)
                {
                    continue;
                }

                //Debug.LogError($"111Buff_30000019 {buffArgs}");
                if (refData.cdfeDropsData[inData.dropsEntities[i]].id == buffArgs.args1 ||
                    refData.cdfeDropsData[inData.dropsEntities[i]].id == buffArgs.args2 ||
                    refData.cdfeDropsData[inData.dropsEntities[i]].id == buffArgs.args3 ||
                    refData.cdfeDropsData[inData.dropsEntities[i]].id == buffArgs.args4)
                {
                    var dropData = refData.cdfeDropsData[inData.dropsEntities[i]];
                    dropData.isLooting = true;
                    refData.cdfeDropsData[inData.dropsEntities[i]] = dropData;
                }
            }
            //inData.cdfeDropsData.HasComponent()
        }
