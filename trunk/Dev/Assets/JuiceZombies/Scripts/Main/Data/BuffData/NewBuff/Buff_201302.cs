...
namespace Main
{
    //元素专用 地形火 障碍物木
    //new 
    public partial struct Buff_201301 : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        public void OnOccur(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            tickTime = (int)((argsNew.args1.y+argsNew.args1.x)/1000f/inData.fdT);
        ...
        public void OnTick(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            for (int i = 0; i < entities.Length; i++)
            {
                ...
                var limitDis = refData.cdfeLocalTransform[carrier].Scale;
                var pos = refData.cdfeLocalTransform[carrier].Position;
                if (refData.cdfeChaStats.HasComponent(entity) && !inData.cdfeMapElementData.HasComponent(entity) && refData.storageInfoFromEntity.Exists(entity))
                {
                    var dis = math.distance(refData.cdfeLocalTransform[entity].Position, pos);
                    if (dis <= limitDis)
                    {
                        refData.ecb.AppendToBuffer<Buff>(inData.sortkey, entity, new Buff_126 { id = 126, permanent = false, carrier = entity, caster = default, duration = argsNew.args1.x / 1000f }.ToBuff());
                    }
                }
            }
