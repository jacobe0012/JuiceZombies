...
namespace Main
{
    ...
    /// 速度转换为推力加成 加等
    /// </summary>
    public partial struct Buff_20001002 : IBuffOld
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        public void OnOccur(ref BuffOldData_ReadWrite refData, in BuffOldData_ReadOnly inData)
        {
            ...
            var prop = temp.chaProperty;
            int ration = (int)(prop.maxMoveSpeed * buffArgs.args0 / 1000f);
            prop.pushForceRatios += ration;

            refData.cdfeChaStats[carrier] = temp;

            Debug.Log($"速度转换为推力加成 {ration}");
        }
