...
namespace Main
{
    //大肉盾
    //new 
    public partial struct Buff_999201 : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        public void OnOccur(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            var shieldCount = monsterfeature.para[0];
            UnityHelper.ChangeProperty(ref refData.ecb, inData.sortkey, ref refData.cdfeChaStats,
                    ref refData.cdfePlayerData, ref refData.cdfePhysicsMass, refData.cdfeLocalTransform, 212000, shieldCount, carrier);
        ...
        public void OnTick(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            int maxLevel = monsterfeature.para[2];
           

            if (level < maxLevel)
            {
                var curShieldCount = refData.cdfeChaStats[carrier].chaProperty.shieldCount;

                UnityHelper.ChangeProperty(ref refData.ecb, inData.sortkey, ref refData.cdfeChaStats,
                    ref refData.cdfePlayerData, ref refData.cdfePhysicsMass, refData.cdfeLocalTransform, 212000, 1, carrier);
                level++;
            }
