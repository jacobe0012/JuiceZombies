...
{
    //濒死buff
    public partial struct Buff_BeforeBeKilled : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        {
            for (int i = 0; i < refData.buff.Length; i++)
            {
                if (refData.buff[i].CurrentTypeId == Buff.TypeId.Buff_BeforeBeKilled)
                {
                    if (refData.buff[i].BuffArgsNew_12.args1.y > argsNew.args1.y)
                    {
                        return;
                    }
                }
            ...
            temp.chaResource.hp = 1;
            refData.cdfeChaStats[carrier]=temp;
            UnityEngine.Debug.LogError($"id:{argsNew.args1.x}");
            BuffHelper.AddOnceCastSkill(ref refData.ecb, argsNew.args1.x, carrier, inData.sortkey);
            duration = 0.02f;
            permanent = false;
        }
