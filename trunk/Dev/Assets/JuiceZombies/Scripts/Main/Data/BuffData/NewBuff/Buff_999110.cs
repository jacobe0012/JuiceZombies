...
namespace Main
{
    //殉道者
    //new 
    public partial struct Buff_999110 : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        public void OnBeHurt(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            var distance = monsterfeature.para[1]/1000f;
            var attRations = monsterfeature.para[2];
            var hpRations = monsterfeature.para[0];
            var skillattackDuration= monsterfeature.para[3]/1000f;
            if (refData.cdfeChaStats[carrier].chaResource.hp <=
                refData.cdfeChaStats[carrier].chaProperty.maxHp * hpRations / 10000f)
            {
                var pos = inData.cdfeLocalTransform[carrier].Position;
                Debug.Log($"110 isActive 11111111111111111111111111");
                ...
                //算是特效？
                var prefab = inData.prefabMapData.prefabHashMap["model_bullet_11511001"];
                var ins = refData.ecb.Instantiate(inData.sortkey, prefab);
                var newpos = new LocalTransform
                {
                    Position = pos,

                    Scale = distance * inData.cdfeLocalTransform[prefab].Scale,
                ...
                refData.ecb.AddComponent(inData.sortkey, ins, new TargetData
                {
                    tick = 0,
                    AttackWith = (uint)(8+16+32),
                    BelongsTo=0
                });
            }
        }
