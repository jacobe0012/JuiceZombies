...
namespace Main
{
    //魅惑    状态
    //new 
    public partial struct Buff_116 : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        public void OnRemoved(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            if (refData.cdfeChaStats.HasComponent(carrier))
            {
                //SetImmuneControl(ref refData, inData);
                var temp = refData.cdfeChaStats[carrier];
                temp.chaControlState.forcedmove.y = false;
        ...
        {
            var temp = refData.cdfeChaStats[carrier];
            temp.chaProperty.maxMoveSpeed = (int)(fixedSpeed * 1000);
            refData.cdfeChaStats[carrier] = temp;

            var agent = refData.cdfeAgentLocomotion[carrier];
            agent.Speed = fixedSpeed * inData.gameTimeData.logicTime.gameTimeScale;
