...
namespace Main
{
    //钢铁之躯
    //new 
    public partial struct Buff_999202 : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        public void OnOccur(ref BuffData_ReadWrite refData, in BuffData_ReadOnly inData)
        {
            ...
            UnityHelper.ChangeProperty(ref refData.ecb, inData.sortkey, ref refData.cdfeChaStats,
                ...
                massRations, carrier);
            UnityHelper.ChangeProperty(ref refData.ecb, inData.sortkey, ref refData.cdfeChaStats,
                ref refData.cdfePlayerData, ref refData.cdfePhysicsMass, refData.cdfeLocalTransform, 202020, hpRations,
                carrier);
            var entityGroupData = inData.cdfeEntityGroupData[inData.entity];
            
            UnityHelper.EnableShaderFX(ref refData.ecb, inData.sortkey,
                entityGroupData.renderingEntity, id);
            // foreach (var child in inData.bfeChild)
            // {
            //     if (inData.cdfeJiYuSort.HasComponent(child.Value) &&
            //         !inData.cdfeWeaponData.HasComponent(child.Value))
            //     {
            //         break;
            //     }
            // }
