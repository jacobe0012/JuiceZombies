...
namespace Main
{
    //电击   状态
    //new 
    public partial struct Buff_110 : IBuff
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        {
            var buffs = refData.buff;
            for (int i = 0; i < buffs.Length; i++)
            {
                if (buffs[i].CurrentTypeId == Buff.TypeId.Buff_110)
                {
                    for (int j = i + 1; j < buffs.Length; j++)
                    {
                        if (buffs[j].CurrentTypeId == Buff.TypeId.Buff_110)
                        {
                            var buff = buffs[j];
                            buff.Single_3 = 0;
                            buffs[j] = buff;
        ...
        private void InitPar(ref BuffData_ReadWrite refData, BuffData_ReadOnly inData)
        {
            ...
            for (int i = 0; i < battleConstConfig.Length; i++)
            {
                if (battleConstConfig[i].constantName == (FixedString128Bytes)"status_electric_paralysis")
                {
                    eletricDuration = battleConstConfig[i].constantValue / 1000f;
                }
            }
        }
    }
