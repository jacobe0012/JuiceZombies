...
namespace Main
{
    ...
    /// 死亡自爆 用于炸药桶等能自爆的障碍物
    /// </summary>
    public partial struct Buff_DieToBoom : IBuffOld
    {
        ///<summary>0
        ///buff的id
        ///</summary>
        ...
        public void OnBeKilled(ref BuffOldData_ReadWrite refData, in BuffOldData_ReadOnly inData)
        {
            ...
            // Debug.Log($"OnBeDie carrier{carrier}");

            float args0 = 0, args1 = 0, args2 = 0, args3 = 0;
            uint target = 0;
            var maxHP = refData.cdfeChaStats[carrier].chaProperty.maxHp;
            ref var eventsMap = ref inData.globalConfigData.value.Value.configTbevent_0s.configTbevent_0s;
            ...
            // SetColliderTarget(ref refData, prefab, target);
            ...
            var newpos = new LocalTransform
            {
                ...
                //TODO:大小*10
                Scale = (args0 + 10) * inData.cdfeLocalTransform[prefab].Scale,
            ...
            refData.ecb.AppendToBuffer(inData.sortkey, ins, new SkillHitEffectBuffer
            {
                ...
                {
                    c0 = new float3(args1 * 10000, 0, 0),
                    c1 = default,
                    c2 = default,
                    c3 = new float3(target, 0, 0)
                }
            });

            int trueDamage =
                (int)args2 + (int)((args3 / 10000f) * maxHP);
            Debug.Log($"OnBeDie trueDamage{trueDamage}，args1:{args1},args2:{args2},args3:{args3},maxHP:{maxHP}");
            refData.ecb.AppendToBuffer(inData.sortkey, ins, new SkillHitEffectBuffer
            {
                buffID = 30000003,
                buffArgs = new float3x4
                {
                    c0 = new float3(trueDamage * 300000, 0, 0),
                    c1 = default,
                    c2 = default,
                    c3 = new float3(target, 0, 0)
                }
