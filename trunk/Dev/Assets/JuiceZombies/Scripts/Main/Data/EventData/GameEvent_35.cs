...
namespace Main
{
    ...
    ///变更环境
    /// </summary>
    public partial struct GameEvent_35 : IGameEvent
    {
        /// <summary>
        ///事件id 0
        /// </summary>
        ...
        {
            ChangeEnvironment(ref refData, inData);
        }

        private void ChangeEnvironment(ref GameEventData_ReadWrite refData, GameEventData_ReadOnly inData)
        {
            var temp = refData.cdfeEnviromentData[inData.wbe];
            if (args123.x / 100 == 1)
            {
                temp.env.weather = args123.x;
            }
            else if (args123.x / 100 == 2)
            {
                temp.env.time = args123.x;
            }

            refData.cdfeEnviromentData[inData.wbe] = temp;
        }
