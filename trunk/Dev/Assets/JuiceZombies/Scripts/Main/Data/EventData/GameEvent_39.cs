...
{
    /// <summary>
    ///每次出现商店将额外获得【参数1】次免费刷新次数，仅对此次商店生效，不可保留。
    /// </summary>
    public partial struct GameEvent_39 : IGameEvent
    {
        /// <summary>
        ///事件id 0
        /// </summary>
        ...
        {
            ChangeTempCount(ref refData, inData);
        }

        private void ChangeTempCount(ref GameEventData_ReadWrite refData, GameEventData_ReadOnly inData)
        {

            isActive = true;

            //if (refData.cdfePlayerData.HasComponent(inData.selefEntity))
            //{
            //    var temp = refData.cdfePlayerData[inData.selefEntity];
            //    temp.playerOtherData.tempRefreshCount = args123.x;
            //    refData.cdfePlayerData[inData.selefEntity] = temp;
            //}
            ...
            //{
            //    type = 46,
            //    args = new float4(args123.x, float3.zero)
            //});
