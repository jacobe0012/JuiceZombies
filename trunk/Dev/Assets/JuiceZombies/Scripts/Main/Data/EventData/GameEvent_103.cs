...
namespace Main
{
    ...
    ///环境事件 雪
    /// </summary>
    public partial struct GameEvent_103 : IGameEvent
    {
        /// <summary>
        ///事件id 0
        /// </summary>
        ...
        public void OnOccur(ref GameEventData_ReadWrite refData, in GameEventData_ReadOnly inData)
        {
            ...
            if(index>=0)
            {
                 triggerGap = environmentConfig[index].para[0]/1000f;
                 maxLimit = 999999;
            }
        }
        ...
        public void OnPerGap(ref GameEventData_ReadWrite refData, in GameEventData_ReadOnly inData)
        {
            if (refData.cdfeEnviromentData[inData.wbe].env.weather == id)
            {
                ...
                for (int i = 0; i < inData.entities.Length; i++)
                {
                    ...
                    {
                        if (inData.cdfeMapElementData[inData.entities[i]].elementID == 1006)
                        {
                            var centerPos = new float2(inData.cdfeLocalTransform[inData.entities[i]].Position.x, inData.cdfeLocalTransform[inData.entities[i]].Position.y);
                ...
                for (int i = 0; i < num; i++)
                {
                    ...
                    BuffHelper.GenerateMapModule(inData.cdfePostTransformMatrix, inData.cdfeLocalTransform[inData.player].Position,inData.gameOthersData, ref refData.ecb, inData.config, inData.prefabMapData, inData.sortKey, 1002, new float3(point.xy, 0), ref ins,isinBoss, bossScenSize);
                }
            }
        }
        ...
        {
            if (refData.cdfeEnviromentData[inData.wbe].env.weather == id&&durationTick%((int)(3/inData.fDT))==0)
            {
                var playerPos = inData.cdfeLocalTransform[inData.player].Position;
                var mapInfo = inData.gameOthersData.mapData.mapSize;
                var smallRect = new MathHelper.Rectangle
                ...
                for (int i = 0; i < inData.entities.Length; i++)
                {
                    if (inData.cdfeMapElementData.HasComponent(inData.entities[i]))
                    {
                        if (inData.cdfeMapElementData[inData.entities[i]].elementID == 1002)
                        {
                            ...
                            if (smallRect.Contains(centerPos))
                            {
                                refData.ecb.AddComponent(inData.sortKey, inData.entities[i], new TimeToDieData { duration = 0.02f });
                                Entity newIns = default;
                                bool isinBoss = refData.cdfePlayerData[inData.player].playerOtherData.isBossFight;
                                float bossScenSize = refData.cdfePlayerData[inData.player].playerOtherData.bossScenePos.y;
                                BuffHelper.GenerateMapModule(inData.cdfePostTransformMatrix, inData.cdfeLocalTransform[inData.player].Position, inData.gameOthersData,ref refData.ecb, inData.config, inData.prefabMapData, inData.sortKey, 1006, inData.cdfeLocalTransform[inData.entities[i]].Position,ref newIns, isinBoss,bossScenSize);
                                refData.ecb.AddComponent(inData.sortKey, inData.entities[i], new TimeToDieData { duration = 0.02f });
                            }
            ...
            //    {
            //        if (buffs[i].CurrentTypeId == Buff.TypeId.Buff_333101)
            //        {
            //            isNeedAdd = false;
            //            break;
            //        }
            ...
            //    {
            //        UnityHelper.ChangeProperty(ref refData.ecb, inData.sortkey, ref refData.cdfeChaStats,
                    //ref refData.cdfePlayerData, refData.cdfeLocalTransform, 209020, pushRations, entity);
            //    }
            //}
        }
