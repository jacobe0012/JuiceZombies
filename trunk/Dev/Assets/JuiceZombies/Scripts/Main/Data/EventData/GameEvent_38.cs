...
namespace Main
{
    ...
    ///切换boss场景
    /// </summary>
    public partial struct GameEvent_38 : IGameEvent
    {
        /// <summary>
        ///事件id 0
        /// </summary>
        ...
        {
            SwitchBossScene(ref refData, inData);
        }

        private void SwitchBossScene(ref GameEventData_ReadWrite refData, GameEventData_ReadOnly inData)
        {
            // var pos = BuffHelper.GenerateBossMap(ref refData.ecb, inData.sortKey,
            //     inData.config,
            //     inData.prefabMapData, args123.x, inData.gameOthersData.mapData.mapID, out var groupEntity,
            //     out var surfaceEntity);
            // var playerTran = inData.cdfeLocalTransform[inData.player];
            // playerTran.Position = pos;
            // refData.ecb.SetComponent(inData.sortKey, inData.player, playerTran);

            //UnityHelper.TryCreateAudioClip(inData.config, inData.gameOthersData, 2003);
        }
