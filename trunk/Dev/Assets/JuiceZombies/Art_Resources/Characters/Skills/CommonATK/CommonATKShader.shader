Shader "Unlit/CommonATKShader"
{
    Properties
    {
        _MainTex("_MainTex", 2D) = "white" {}
        _TrailTex("_TrailTex", 2D) = "white" {}
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct WarningDataStruct
            {
                float4 WarningData;
                float3 MonsterGeneralAttackData;
            };
            ...
            StructuredBuffer<WarningDataStruct> _WarningDataBuffer;
#endif
            ...
            float4 UnlitFragment(Varyings varyings, uint instanceID : SV_InstanceID) : SV_Target
            {
                ...
                WarningDataStruct warningData = _WarningDataBuffer[propertyIndex];
#else
                float4 uvAtlas = float4(1, 1, 0, 0);
                WarningDataStruct warningData;
                warningData.WarningData = float4(1, 1, 1, 1);
                warningData.MonsterGeneralAttackData = float3(1, 1, 1);
#endif
                //test property
                // float alpha = 135;
                // alpha = PI * alpha / 180;
                ...
                //MonsterGeneralAttackData x:持续时间 y:生成时间 z:扇形角度

                float alpha = warningData.MonsterGeneralAttackData.z;
                alpha = PI * alpha / 180;
                float cycleT = warningData.MonsterGeneralAttackData.x;
                float initTime = warningData.MonsterGeneralAttackData.y;
                float nowTime = _Time.y - initTime;

                float theta = alpha / 2 - nowTime * alpha / cycleT;
                float beta = alpha * nowTime / (1.28 * cycleT);
                //float r = 0.5 * (0.5 * cos(2 * PI * nowTime / (1.5 * cycleT)) + 0.5) + 0.25;
                //r = 0.32;
                //r = 0;
                float r = 0.35;
                ...
                //varyings.uv = uvTransform(varyings.uv, PI + _Time.x * -100, _Time.x * 50, 0.3, 1);
                varyings.uv = uvTransform(varyings.uv, theta, beta, r, 1, alpha);

                float4 texColor = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, varyings.uv);
                float4 tex1Color = SAMPLE_TEXTURE2D(_TrailTex, sampler_TrailTex, varyings.uv);
                tex1Color = tex1Color * step(0, nowTime);
                clip(tex1Color.w - 0.5);
                return tex1Color;
            }
            ENDHLSL
        }
