Shader "Unlit/HaloShader"
{
    Properties
    {
        ...
        //[HDR]_HaloColor("HaloColor", Color) = (0.0, 0.0, 0.0, 0)
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 UnlitFragment(Varyings varyings, uint instanceID : SV_InstanceID) : SV_Target
            {
                ...
                float2 relativePos = varyings.uv * 2 - 1;
                float distanceToCenter = length(relativePos - hexagonCenter);

                //float distanceToCenter = distance(varyings.uv, float4(0.5, 0.5, 0, 0));
                float ripple = sin(_Time.y * 2.5 - distanceToCenter * 5);

                //texColor.rgb = texColor.rgb * _HaloColor.rgb;
                texColor = float4(1.0, 0.2, 0, 1.0);
                //texColor.a = texColor.a * (step(-0.05,ripple) - step(0.05, ripple) + step(0.999,ripple) - step(1, ripple) + step(-1,ripple) - step(-0.999, ripple));
                texColor.a = texColor.a *((smoothstep(-0.2, -0.05, ripple)  - smoothstep(0.05, 0.2, ripple) + smoothstep(0.975, 0.999, ripple) - step(1, ripple) + step(-1, ripple) - smoothstep(-0.999, -0.975, ripple)) * 0.75 + 0.25);
                texColor.a = texColor.a * smoothstep(-1, -0.5, -distanceToCenter);
                //smoothstep(-0.2, -0.05, x)  - smoothstep(0.05, 0.2, x)
                //smoothstep(0.8, 0.999, x) - step(1, x)
                //step(-1, x) - smoothstep(-0.999, -0.8, x)
