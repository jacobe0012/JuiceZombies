Shader "Unlit/LineSkillShader"
{
    Properties
    {
        _MainTex("_MainTex", 2D) = "white" {}
        [HDR]_LineColor("Line Color", Color) = (0.0, 0.0, 0.0, 0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 UnlitFragment(Varyings varyings, uint instanceID : SV_InstanceID) : SV_Target
            {
                ...
                texColor.rgb = texColor.rgb * (_LineColor.rgb * (1 - sin(PI * varyings.uv.x)) + _MiddleColor.rgb * sin(PI * varyings.uv.x));
                clip(texColor.w - 0.5);
                return texColor;
            }
            ENDHLSL
