        ...
        Pass
        {
            ...
            float2 switchNumber(float2 input, float2 uvHelp, int targetNumber, int inputlength, int cut1length, int cut2length, int renderlength)
            {
                ...
                while(i < renderlength + 1)
                {
                    ...
                    if(uvHelp.x >= (renderlength - i) * help + 0.35 - havePoint * helpChangeForPoint / 2 + (pointHelp + pointHadBeenRendered) * helpChangeForPoint
                        && uvHelp.x < (renderlength - i + 1) * help + 0.35 - havePoint * helpChangeForPoint / 2 + pointHadBeenRendered * helpChangeForPoint)
                    {
                        input.x = displayNX * 0.2 + 0.1 * (1 -_CharacterProportion / 1000) + 
                                    0.2 * (_CharacterProportion / 1000) * (input.x - ((renderlength - i) * help) - 0.35 + havePoint * helpChangeForPoint / 2 - pointHadBeenRendered * helpChangeForPoint) / help 
                    ...
                    input.y = input.y * (step(0.35 + havePoint * helpChangeForPoint / 2, uvHelp) 
                                - step(0.65 - havePoint * helpChangeForPoint / 2, uvHelp));
                    i++;
