...
void calculateFrameValuesPos(half3 position,
    Texture2D<float4> animatedBoneMatrices, 
    half2 boneWeights[6], int frameIndex, 
...
void AnimateBlendPos_half(half3 position, 
    half3x4 uvs, Texture2D<float4> animatedBoneMatrices, half4x3 animationState,out half3 positionOut) 
{
    positionOut = half3(0, 0, 0);
    ...
    for(int blendIndex = 0; blendIndex < 4; blendIndex++)
    {
        ...
        if(blendFactor > 0)
        {
            ...
            half prevFrameFrac = 1.0 - transitionNextFrame;
            half frameIndex = animationState[blendIndex][2];
            half3 posOutBefore, posOutAfter;
            calculateFrameValuesPos(position,  animatedBoneMatrices, boneWeights, frameIndex, 
                posOutBefore);
            calculateFrameValuesPos(position,animatedBoneMatrices, boneWeights, frameIndex + 1,
                posOutAfter);
            positionOut += blendFactor * (prevFrameFrac * posOutBefore + transitionNextFrame * posOutAfter);
