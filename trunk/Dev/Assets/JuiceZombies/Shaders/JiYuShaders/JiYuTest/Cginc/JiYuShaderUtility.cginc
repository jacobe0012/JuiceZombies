...
//-----------------------------------------------------------------------
half RemapHalf(half inValue, half inMin, half inMax, half outMin, half outMax){
...
float JiYuSortFunc(float _JiYuPivot,int2 _JiYuSort,float4 _sortingGlobalData,float4x4 unity_MatrixMVP)
{
  				float4 mvp3 = mul(unity_MatrixMVP, float4(0,RemapFloat(_JiYuPivot, 0, 1, -1, 1),0,1));
				//float4 mvp3 = mul(unity_MatrixMVP, float4(0,0,0,1));
                ...
                return
                    ...
                    + _sortingGlobalData.y * saturate(RemapFloat(screenClipSpacePos.y, -1, 1, 0, 1));  // screen y pos offset
}
