Shader "AllIn1SpriteShader/AllIn1Urp2dRendererFrameAnimTest"
{
    Properties
    {
		...
		[HideInInspector] _TimeScale("TimeScale", Float) = 10.0  //185
		_JiYuStartTime("JiYu StartTime", Float) = 0		//186
		_JiYuFrameAnimSpeed("JiYuFrameAnimSpeed", Float) = 1		//187
		_FrameRow("FrameRow", Int) = 4		//188
		_FrameCol("FrameCol", Int) = 4		//189
		_JiYuFlip("JiYu Flip",Vector) = (0, 0, 0, 0)		//190
		_JiYuSort("JiYu Sort",Vector) = (0, 0, 0, 0)		//191
		_JiYuPivot("JiYu Pivot",Range(0, 1)) = 0.5		//192
		_JiYuAnimIndex("JiYu AnimIndex",Int) = 0		//193
		_JiYuFrameAnimLoop("JiYu FrameAnimLoop",Int) = 1	//194			
    }
        ...
        Pass //Main Light Shapes pass
        {
			...
			CBUFFER_END
				...
				UNITY_DOTS_INSTANCING_START(MaterialPropertyMetadata)
					...
					//UNITY_DOTS_INSTANCED_PROP(half4, _MainTex_ST)
					UNITY_DOTS_INSTANCED_PROP(int2, _JiYuFlip)
					UNITY_DOTS_INSTANCED_PROP(int2, _JiYuSort)
					UNITY_DOTS_INSTANCED_PROP(half, _JiYuPivot)	
					UNITY_DOTS_INSTANCED_PROP(int, _JiYuAnimIndex)
					UNITY_DOTS_INSTANCED_PROP(half, _JiYuFrameAnimSpeed)
					UNITY_DOTS_INSTANCED_PROP(int, _JiYuFrameAnimLoop)	
					UNITY_DOTS_INSTANCED_PROP(half, _JiYuStartTime)	
					UNITY_DOTS_INSTANCED_PROP(half, _RandomSeed)			
							
				UNITY_DOTS_INSTANCING_END(MaterialPropertyMetadata)

				//#define _Color UNITY_ACCESS_DOTS_INSTANCED_PROP_WITH_DEFAULT(float4, _Color)
				//#define _MainTex_ST UNITY_ACCESS_DOTS_INSTANCED_PROP_WITH_DEFAULT(half4, _MainTex_ST)
                ...
                #define _JiYuPivot UNITY_ACCESS_DOTS_INSTANCED_PROP_WITH_DEFAULT(half, _JiYuPivot)	
 				#define _JiYuAnimIndex UNITY_ACCESS_DOTS_INSTANCED_PROP_WITH_DEFAULT(int, _JiYuAnimIndex)
                #define _JiYuFrameAnimSpeed UNITY_ACCESS_DOTS_INSTANCED_PROP_WITH_DEFAULT(half, _JiYuFrameAnimSpeed)
                #define _JiYuFrameAnimLoop UNITY_ACCESS_DOTS_INSTANCED_PROP_WITH_DEFAULT(int, _JiYuFrameAnimLoop)
                #define _JiYuStartTime UNITY_ACCESS_DOTS_INSTANCED_PROP_WITH_DEFAULT(half, _JiYuStartTime)
                #define _RandomSeed UNITY_ACCESS_DOTS_INSTANCED_PROP_WITH_DEFAULT(half, _RandomSeed)
			...
			*/
            ...
            v2f CombinedShapeLightVertex(Attributes v)
            {
				...
				int rowIndex =(uint)(offsetIndex-1) / (uint)_FrameCol; 
				int colIndex = (uint)(offsetIndex-1) % (uint)_FrameCol;

				float2 offset =_JiYuAnimIndex>0? float2(_MainTex_ST.z+colIndex,_MainTex_ST.w -rowIndex):0;

				o.uv = v.uv.xy * _MainTex_ST.xy + _MainTex_ST.zw +offset;

                o.uv.x = _JiYuFlip.x >= 1 ? (1.0 - o.uv.x): o.uv.x;
                o.uv.y = _JiYuFlip.y >= 1 ? (1.0 - o.uv.y): o.uv.y;
            ...
            half4 CombinedShapeLightFragment(v2f i) : SV_Target
            {
				...
				// Speed of animation
				float speed=_JiYuAnimIndex>0 ? 0 : _JiYuFrameAnimSpeed;
				...
				float currentRow =fbcurrenttileindex2_g12/(float)_FrameCol;
				...
				// Flipbook UV
				half2 fbuv2_g12 = i.uv.xy * fbtiling2_g12 + fboffset2_g12;
				// *** END Flipbook UV Animation vars ***
				half4 col = tex2D( _MainTex, fbuv2_g12 )* _Color;

				if (_JiYuFrameAnimLoop<=0 && _JiYuAnimIndex<=0 ) {
					
					float duration =(float)_FrameCol * (float)_FrameRow * (1.0 /(_TimeScale*_JiYuFrameAnimSpeed+1.0));
					//_JiYuCurTime =_TimeParameters.x - _JiYuStartTime;
					//duration = duration - _JiYuCurTime;
					col.w = _JiYuStartTime >= duration ? 0.0 : col.w;
				}
				...
				//float2 uvRect = fbuv2_g12;
				//half2 center = half2(0.5*uvRect.x, 0.5*uvRect.y);
				half2 uvMin = half2(fblinearindextox2_g12 * fbcolsoffset2_g12, currentRow * fbrowsoffset2_g12);
				half2 uvMax = half2((fblinearindextox2_g12 + 1) * fbcolsoffset2_g12, (currentRow + 1) * fbrowsoffset2_g12);
				//_MinXUV =uvMin.x;
				//_MinYUV =uvMin.y;
				//_MaxXUV =uvMax.x;
				//_MaxYUV =uvMax.y;
				...
				#if ATLAS_ON
				//center = half2((_MaxXUV + _MinXUV) / 2.0, (_MaxYUV + _MinYUV) / 2.0);
				center = half2(0.5*uvRect.x, 0.5*uvRect.y);
	...
	CustomEditor "AllIn12DRendererFrameAnimMaterialInspector"
